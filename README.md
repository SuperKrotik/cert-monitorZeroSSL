...
- Email-уведомления через SMTP (сервисный Mailfence-аккаунт: SSL 465).
...
- **Отдельный сервисный Mailfence-аккаунт** (не личный), созданный только для писем программы
  (`https://mailfence.com` — бесплатно, без телефона; SMTP `smtp.mailfence.com:465` SSL).
...
| `notify.smtp` | сервисный Mailfence (логин + пароль ящика) | — |
