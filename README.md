...
- Email-уведомления через SMTP (российский сервис SMTP.BZ: доступен из РФ).
...
- Аккаунт **SMTP.BZ** (`https://smtp.bz/register`) для отправки писем: бесплатный тариф,
  российский сервис, доступен из РФ без блокировок. Данные подключения — в разделе
  «Подключение и квота» (хост, порт, логин, пароль).
...
| `notify.smtp` | SMTP.BZ relay (хост/порт/логин/пароль из ЛК) | — |
